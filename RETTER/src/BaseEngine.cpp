...
, m_ppDisplayableObjects(0) // Intialising the vector with 0 size
...
{
	//If the object is NULL, it will not be added to the vector
	if (pObject != NULL){
		//Appending the pObject to the end of the list
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	for (DisplayableObject *pObject : m_ppDisplayableObjects){
		pObject->DoUpdate(iCurrentTime);
		if (m_iDrawableObjectsChanged){
			return; // Abort! Something changed in the vector
		}
	}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	int iReturn = 0;
	for (DisplayableObject *pObject : m_ppDisplayableObjects){
		if (pObject->Notify(iSignalNumber) != 0){
			iReturn++;
		}
	}
	return iReturn;
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	int iReturn = 0;
	for (DisplayableObject *pObject : m_ppDisplayableObjects){
		iReturn += pObject->Notify(iSignalNumber);
	}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	int iReturn = INT_MIN;
	for (DisplayableObject *pObject : m_ppDisplayableObjects){
		int ival = pObject->Notify(iSignalNumber);
		if (ival > iReturn){
			iReturn = ival;
		}
